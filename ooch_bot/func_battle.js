 ...
 * @returns 
 */
export async function generate_battle_user(type, options) {
    ...
    if (Object.prototype.hasOwnProperty.call(options, "team")) {
...
export async function setup_battle (users, weather, oochabux, turn_timer, allow_items, give_rewards, allow_run, fake_battle = false, scale_to_level = false, battle_bg = 'battle_bg_tutorial', is_online = false) {
    ...
    let battleId = generate_battle_id();
        ...
        for(let user of battleDataObj.users.values()) {
            for (let i = 0; i < user.party.length; i++) {
                ...
                if (scale_to_level !== false) {
                    let new_stats = get_stats(user.party[i].id, scale_to_level, user.party[i].stats.hp_iv, user.party[i].stats.atk_iv, user.party[i].stats.def_iv, user.party[i].stats.spd_iv);
    ...
    if(switch_in_text != ''){
        switch_in_embed = battle_embed_create(switch_in_text)
    }
    ...
    for (let [id, user] of battleDataObj.users.entries()) {
        ...
        types_string = ` ${type_to_emote(active_ooch.type)}`;
        ...
        if (users[id].is_player) {
            ...
            let startBattleEmbeds = [battle_embed_create(`${sendOutText}`)];
            if (switch_in_embed != false) startBattleEmbeds.push(switch_in_embed);
            startBattleEmbeds.push(generate_round_start_embed(battleDataObj, stancesEnabled));
            ...
            case BattleAi.Basic: 
                if (battle_data.turn_counter % random(2, 3) == 0) { //Allow Setup
                    ...
                    if(damage_moves.length > 0){
                        move_id = damage_moves[random(10) < 6 ? 0 : random(damage_moves.length - 1)].move_id;
                    } else {
                        move_id = move_intentions[random(10) < 6 ? 0 : random(move_intentions.length - 1)].move_id;
                    }
        ...
        if((u.team_id != user.team_id || every_target_list.includes(ooch_to.ability)) && u.party[u.active_slot].alive){
            ...
            switch (ooch_enemy.ability) {
                case Ability.Alert: //Increases atk if a new enemy mon switches in
                    ...
                    string_to_send += `\n--- ${modify_stat(ooch_enemy, Stats.Attack, 1)}\n`;
                break;
                case Ability.Duplicant: //Copies the ability of the newly switched-in mon
                    if(![Ability.InvalidEntry].includes(ooch_to.ability)) { //the array is for abilities that we don't want copied
                        ...
                        string_to_send += `\n--- ${ooch_enemy.emote} **${ooch_enemy.nickname}** ability was changed to **${startCase(ability_data.get(`${ooch_to.ability}`, 'name'))}**!\n`;
                    }
                break;
                ...
                case Ability.Pursuer:
                    if(ooch_from.current_hp > 1 && slot_from != slot_to && slot_from != -1) {
                        ...
                        ooch_from.current_hp = _max([Math.floor(ooch_from.current_hp - (ooch_from.stats.hp / 5)), 1]); //This should never kill the mon swapping out
            ...
            switch (ooch_to.ability) {
                ...
                case Ability.Withering: //Lowers enemy DEF 1 stage
                    ...
                    string_to_send += `\n--- ${modify_stat(ooch_enemy, Stats.Defense, -1)}\n`;
                break;
            }
        }
    }
    ...
    switch(battle_data.field_effect) {
        case FieldEffect.JaggedGround:
            if(!ooch_to.type.includes(OochType.Stone)){
                ...
                ooch_to.current_hp = clamp(Math.floor(ooch_to.current_hp - ooch_to.stats.hp * 0.1), 1, ooch_to.stats.hp);
    ...
    if(prism_result == true){
        return_string += `\n*The prism pulses once...*\n*The prism pulses twice..!*\n\n**You successfully caught the wild ${ooch_target.nickname}!**`;
            ...
            if (ooch_plr.level != 50) {
                return_string += `\n\n${monster_data.get(`${ooch_plr.id}`, 'emote')} **${ooch_plr.nickname}** earned **${exp_earned_main} exp!**`;
...
export async function action_process_stance_change(battle_data, action) {
    ...
    let return_string = `${active_ooch.emote} **${active_ooch.nickname}** shifted its stance to the **${stance_data.get(`${stance_to}`, 'name')} Stance!**`;
    ...
    for(let user of battle_data.users) {
        ...
        if(user.defeated){
            ...
            if(active_ooch.current_hp > 0){
                ...
                active_ooch = update_tame_value(active_ooch, random(-20, -10));
            }
                ...
                for(let other_user of battle_data.users){
                    if((other_user.user_type == UserType.Player) && (other_user.user_index != user.user_index)){
                        ...
                        if (other_ooch.level < max_level) { 
                            finish_string_to_send += `\n\n${monster_data.get(`${other_ooch.id}`, 'emote')} **${other_ooch.nickname}** earned **${exp_main} EXP!**` + 
                        ...
                        for (let i = 0; i < ooch_party.length; i++) {
                            if (i == other_user.active_slot) { 
                                ...
                                ooch_party[i] = update_tame_value(ooch_party[i], random(1, 3));
                            } 
                            else { 
 ...
 */
export function battle_calc_exp(ooch, bonus_multiplier = 1) {
    let mon_data = monster_data.get(`${ooch.id}`);
    let evo_stage = mon_data.evo_stage;
    let exp_multiplier = 0.90
    let level = ooch.level;
    
    return Math.round(
        ((bonus_multiplier * (1.025 ** level) * (2 ** evo_stage) * 8 * level * (random(90, 100)/100)) + 20) * exp_multiplier + //Base EXP given at all levels
 ...
 */
export function get_random_targets(users, my_team_id, my_user_index, max_num = 1, other_team = true, same_team = false, allow_self = false) {
    let options = []
    for(let user of users){
 ...
 */
export function get_team_users(users, team_id) {
 ...
 */
export function get_stats(species_id, level, hp_iv, atk_iv, def_iv, spd_iv) {
    let base_hp = monster_data.get(`${species_id}`, Stats.HP)
    let base_atk = monster_data.get(`${species_id}`, Stats.Attack)
    let base_def = monster_data.get(`${species_id}`, Stats.Defense)
    let base_spd = monster_data.get(`${species_id}`, Stats.Speed)
 ...
 */
export function type_effectiveness(attack_type, target_type) {
    let multiplier = 1;
    let string = '';
...
export function hp_chunks_lost(hp_max, hp_before, hp_current, chunk_percentage){
    let chunks_lost = 0;
    hp_before /= hp_max;
    hp_current /= hp_max;
    ...
    switch(ooch.ability) {
        case Ability.EscalationProtocol:
            ...
            if(chunks_lost > 0 && ooch.current_hp > 0){
                ...
                ability_text += `\n--- ${modify_stat(ooch, Stats.Attack, chunks_lost)}`;
                ability_text += `\n--- ${modify_stat(ooch, Stats.Defense, chunks_lost)}`;
                ability_text += `\n--- ${modify_stat(ooch, Stats.Speed, chunks_lost)}\n`;
            }
        break;
        ...
        case Ability.Stealthy:
            if(!slot_info.this_turn_did_damage){
                ...
                ability_text += `\n--- ${add_status_effect(ooch, Status.Focus, slot_info)}\n`;
            }
        break;
        case Ability.Fleeting:
        ...
        switch (ooch.ability) {
            case Ability.Spectral:
                ...
                if (ooch.type == ooch.og_type) {
                    ...
                    ability_text += `\n--- ${ooch.emote} **${ooch.nickname}** changed its type back to ${type_to_emote(ooch.type)} **${ooch.type.map(v => capitalize(v)).join(' | ')}**.\n`
                }
            break;
 ...
 */
export function battle_calc_damage(move_damage, move_type, ooch_attacker, ooch_defender, turn_count) {
    ...
    if (ooch_attacker.ability == Ability.Bipolar) {
        attacker_atk_stat = ooch_attacker.stats.def * get_stat_multiplier(ooch_attacker.stats.def_mul);
    }
    ...
    * (ooch_attacker.ability == Ability.Corrosive ? (clamp(defender_def_stat, 0, 300) / 10) + 1 : 1)
    ...
    * attacker_atk_stat / defender_def_stat) 
    ...
    damage = clamp(damage, 1, ooch_defender.current_hp)
    return damage;
}
...
export async function attack(battle_data, user_index_attacker, user_index_defender, atk_id) {
    ...
    let attacker_emote = monster_data.get(`${attacker.id}`, 'emote');
    let defender_emote = monster_data.get(`${defender.id}`, 'emote');
    ...
    if (move_effects.some(effect => effect.status === 'random')) move_type = sample(defender.type);
    ...
    let chance_to_hit = move_accuracy/100 * get_stat_multiplier(attacker.stats.acc_mul - defender.stats.eva_mul, 4) * status_blind
                ...
                if (Math.random() < eff.chance/100 && eff.chance > 0) { 
                    ...
                    if (typeof eff.status == "number") {
                        ...
                        for(let s of status_adds){
                            defender_field_text += `\n--- ${add_status_effect(status_target, s, slot_defender)}`;
                        }
                    ...
                    // +_def_1 is the format
                        ...
                        defender_field_text += `\n--- ${modify_stat(status_target, status_split[1], stage_change)}`;
    ...
    if (defender.current_hp <= 0) {
        ...
        switch (defender.ability) {
            case Ability.Haunted:
                ...
                defender_field_text += `\n--- ${add_status_effect(attacker, Status.Doom, slot_attacker)}`
            break;
            case Ability.Sporespray:
                defender_field_text += `\n${defender_emote} **${defOochName}**'s **Sporespray**:`
                defender_field_text += `\n--- ${add_status_effect(attacker, Status.Infect, slot_attacker)}`
            break;
            case Ability.InvalidEntry:
                // -1 is the ID of i_
                monster_data.inc('-1', 'atk');
                monster_data.inc('-1', 'def');
                monster_data.inc('-1', 'spd');
                monster_data.inc('-1', 'hp');
            ...
            case Ability.Matryoshka:
                if(slot_defender.used_ability_matryoshka == false){
                    ...
                    defender.current_hp = ceil(defender.stats.hp / 2);
 ...
 */
export function modify_stat(ooch, stat, stage, set = false) {
    let min_stage = -8;
    let max_stage = 8;

    let val_name = `${stat}_mul`;
    let val_get = ooch.stats[val_name];
    let val_set = clamp((set ? stage : val_get + stage), min_stage, max_stage);
    
    ooch.stats[val_name] = val_set;

    let text = `${ooch.emote} **${ooch.nickname}**'s **${toUpper(stat)}**`;
    if(set){
        if(val_get == val_set){
...
export function generate_round_start_embed(battle_data, stances_enabled) {
    ...
    let embed = battle_embed_create(hp_string)
    return(embed);
}
 ...
 */
export function get_stat_multiplier(stage, scalar = 2) {
 ...
 */
export function exp_to_next_level(level) {
 ...
 */
export function level_up(ooch) {
    let level_counter = 0;
    let starting_level = ooch.level;
    let evoData = false;
    let output = ``;
    let possibleMoves = monster_data.get(`${ooch.id}`, 'move_list');
    ...
    if (ooch.level <= 50) {
        ...
        if (ooch.level == monster_data.get(`${ooch.id}`, `evo_lvl`) && monster_data.get(`${ooch.id}`, 'evo_id') != -1) {
...
export function get_stance_options(ooch, include_base = false) {
    ...
    for (let stance of stances) {
        ...
        available_stances.push(stance_data.get(`${stance}`));
    }
