...
var shop_data = {
	"x" : 0,
	"y" : 0,
	"greeting_dialogue" : "Welcome to my store. Buy whatever you need below!",
	"type" : "default",
	"special_items" : [],
	"image" : ""
}
...
func _ready():
	o_shop_greeting.text = shop_data.greeting_dialogue
	o_shop_image_link.text = shop_data.image
	
	if(shop_data.type == "default"):
		o_shop_type.select(0)
	if(shop_data.type == "special"):
		o_shop_type.select(1)
	
	for _info in shop_data.special_items:
		
		var _load = load("res://slot_item.tscn")
		var _obj = _load.instantiate()
		
		_obj.item_id = _info.id
		_obj.item_price = _info.price
...
func _process(delta):
	if typeof(Global.ObjSelected) == typeof(get_instance_id()) and Global.ObjSelected == get_instance_id():
		...
		if dragging:
			...
			shop_data.x = floor(mx/Global.TileSize)
			shop_data.y = floor(my/Global.TileSize)
...
func _on_shop_greeting_text_changed(new_text):
	shop_data.greeting_dialog = new_text

func _on_shop_type_item_selected(index):
	match index:
			0:
				shop_data.type = "default"	
			1:
				shop_data.type = "special"
